	  ...
	  do
		addr_x1=`echo $num_1|awk '{print lshift($1,15)}'`  # fs1 num_1 << 15
			...
			do
				...
				opcode_all=`echo "$opcode_dec $addr_x0 $addr_x1 $addr_x2" |awk '{sum=($1+$2+$3+$4);printf("%08x",sum)}'` # opcode + fd + fs1 +fs2 ->#0x
	    
				echo "$inst f$num_0, f$num_1, f$num_2" >> $good_case_dir/$inst.s
				printf "[ 	]+%s:[ 	]+%8s[ 	]+%s[ 	]+f%s,f%s,f%s\n" $address ${opcode_all} $inst $num_0 $num_1 $num_2 >>$good_case_dir/$inst.d
