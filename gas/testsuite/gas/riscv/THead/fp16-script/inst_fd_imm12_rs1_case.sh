...
pre_error_case_s()
{
	...
	X1=$(($RANDOM%32))
	X2=$(($RANDOM%2048))
	echo "$inst x${X0}, ${X2}(x${X1})"
	echo "$inst f${X0}, ${X2}(f${X1})"
	echo "$inst 29, ${X2}(x${X1})"
	  ...
	  do
		...
		do
			if [[ "$num_2" =~ "-" ]];then
				...
				num_3=1$num_3
				num_3=`echo "obase=10; ibase=2; $num_3" | bc`
				addr_x2=`echo $num_3|awk '{print lshift($1,20)}'`  # imm12 num_2 << 20
			else
				addr_x2=`echo $num_2|awk '{print lshift($1,20)}'`  # imm12 num_2 << 20
			fi
			opcode_dec=`echo $opcode|awk '{print strtonum("0x"$0)}'` # opcode -> dec
			opcode_all=`echo "$opcode_dec $addr_x0 $addr_x1 $addr_x2" |awk '{sum=($1+$2+$3+$4);printf("%08x",sum)}'` # opcode + fd + rs1 + imm12 ->#0x
