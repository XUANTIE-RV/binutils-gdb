...
good_case()
{
	...
	do
	  ...
	  do
	    ...
	    opcode_all=`echo "$opcode_dec $addr_x0 $addr_x1" |awk '{sum=($1+$2+$3);printf("%08x",sum)}'` # opcode + fd + fs1 ->#0x
	    
		echo "$inst f$num_0, f$num_1" >> $good_case_dir/$inst.s
		printf "[ 	]+%s:[ 	]+%8s[ 	]+%s[ 	]+f%s,f%s\n" $address ${opcode_all} $inst $num_0 $num_1 >>$good_case_dir/$inst.d
