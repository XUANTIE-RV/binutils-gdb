...
{
  /* When we integrate zc*, we should check it zca is not set.  */
  if (riscv_opts.rvc
...
riscv_multi_subset_supports (enum riscv_insn_class insn_class)
{
  switch (insn_class)
    ...
    case INSN_CLASS_C_OR_ZCA:
      return (riscv_subset_supports ("c") || riscv_subset_supports ("zca"));
    case INSN_CLASS_ZCB:
      return riscv_subset_supports ("zca") && riscv_subset_supports ("zcb");
    ...
    case INSN_CLASS_D_AND_C_OR_ZCD:
      return riscv_subset_supports ("d")
             && (riscv_subset_supports ("c")
                 || (riscv_subset_supports ("zca")
                     && riscv_subset_supports ("zcd")));
    case INSN_CLASS_ZCMP:
      return riscv_subset_supports ("zca") && !riscv_subset_supports ("zcd")
             && riscv_subset_supports ("zcmp");
    case INSN_CLASS_ZCMT:
      return riscv_subset_supports ("zca") && !riscv_subset_supports ("zcd")
             && riscv_subset_supports ("zcmt");
    case INSN_CLASS_A: return riscv_subset_supports ("a");
    case INSN_CLASS_M: return riscv_subset_supports ("m");
    case INSN_CLASS_ZMMUL: return riscv_subset_supports ("m") || riscv_subset_supports ("zmmul");
    ...
    case INSN_CLASS_ZFA:
      return riscv_subset_supports ("zfa");
    case INSN_CLASS_D_AND_ZFA:
      return riscv_subset_supports ("d")
	     && riscv_subset_supports ("zfa");
    ...
    case INSN_CLASS_H: return riscv_subset_supports ("h");
    case INSN_CLASS_Q: return riscv_subset_supports ("q");
    case INSN_CLASS_Q_AND_ZFH:
      return riscv_subset_supports ("q") && riscv_subset_supports ("zfh");

    case INSN_CLASS_P_OR_ZPN:
      return (riscv_subset_supports ("p")
	      || riscv_subset_supports ("zpn"));
    case INSN_CLASS_P_OR_ZPRVSFEXTRA:
      return (riscv_subset_supports ("p")
	      || riscv_subset_supports ("zprvsfextra"));
    case INSN_CLASS_P_OR_ZPSFOPERAND:
      return (riscv_subset_supports ("p")
	      || riscv_subset_supports ("zpsfoperand"));

    case INSN_CLASS_V_07:
      return (riscv_lookup_subset_version (&riscv_subsets, "v", 0, 7) != NULL);
    case INSN_CLASS_V:
      return (riscv_lookup_subset_version (&riscv_subsets, "v", 1, 0) != NULL);
    case INSN_CLASS_V_AND_F:
      return (riscv_lookup_subset_version (&riscv_subsets, "v", 1, 0)
	      && riscv_subset_supports ("f"));
    case INSN_CLASS_V_OR_ZVAMO:
      return (riscv_subset_supports ("a")
	      && (riscv_lookup_subset_version (&riscv_subsets, "v", 1, 0)
    ...
    case INSN_CLASS_THEADVDOT:
      return (riscv_lookup_subset_version (&riscv_subsets, "v", 1, 0)
	      && riscv_subset_supports ("xtheadvdot"));
    case INSN_CLASS_THEAD_MATRIX:
      return riscv_subset_supports ("xtheadmatrix");
    ...
    case INSN_CLASS_ZICOND:
      return riscv_subset_supports ("zicond");
    case INSN_CLASS_ZICSR:
      return riscv_subset_supports ("zicsr");
    case INSN_CLASS_ZIFENCEI:
      return riscv_subset_supports ("i") || riscv_subset_supports ("zifencei");
    case INSN_CLASS_ZIHINTNTL:
      return riscv_subset_supports ("zihintntl");
    case INSN_CLASS_ZIHINTNTL_AND_C_OR_ZCA:
      return riscv_subset_supports ("zihintntl")
             && (riscv_subset_supports ("c") || riscv_subset_supports ("zca"));
    case INSN_CLASS_ZIHINTPAUSE:
      return riscv_subset_supports ("zihintpause");

    case INSN_CLASS_ZAWRS:
      return riscv_subset_supports ("zawrs");
    ...
    case INSN_CLASS_F_AND_ZFH_OR_ZFBFMIN_OR_ZVFBFWMA:
      return (riscv_subset_supports ("f")
              && (riscv_subset_supports ("zfh")
                  || riscv_subset_supports ("zfbfmin")
                  || riscv_subset_supports ("zvfbfwma")));
    case INSN_CLASS_F_AND_ZFBFMIN:
      return (riscv_subset_supports ("f")
              && riscv_subset_supports ("zfbfmin"));
    case INSN_CLASS_F_AND_V_AND_ZVFBFMIN:
      return (riscv_subset_supports ("f") && riscv_subset_supports ("v")
              && riscv_subset_supports ("zvfbfmin"));
    case INSN_CLASS_F_AND_V_AND_ZVFBFWMA:
      return (riscv_subset_supports ("f") && riscv_subset_supports ("v")
              && riscv_subset_supports ("zvfbfwma"));
...
/* Handle of the OPCODE hash table of RVV 0.7.  */
static struct hash_control *op_v_07_hash = NULL;
...
#define RELAX_BRANCH_FORCE_I(i) (((i) >> 16) & 0x1)
		   ...
		   struct riscv_csr_extra *entry)
{
  ...
  switch (csr_class)
    ...
    case CSR_CLASS_THEAD:
      result = (riscv_subset_supports ("theadc")
		|| riscv_subset_supports ("theade")
		|| riscv_subset_supports ("theadse")
		|| riscv_subset_supports ("theadv"));
      need_check_version = FALSE;
      break;
...
arg_lookup (char **s, const char *const *array, size_t size, unsigned *regnop)
{
  ...
  for (i = 0; i < size; i++)
    if (array[i] != NULL && len == strlen(array[i]) && strncmp (array[i], *s, len) == 0)
      {
	*regnop = i;
	*s += len;
	return TRUE;
...
#define USE_BITS(mask,shift)	(used_bits |= ((insn_t)(mask) << (shift)))
  while (*p)
	...
	switch (c = *p++)
	  ...
	  case '>': used_bits |= ENCODE_RVC_IMM (-1U); break;
	  case '2': used_bits |= ENCODE_RVC_UIMM2 (-1U); break;
      ...
      case 'W': /* Various operands.  */
	{
	  switch (*p++)
	    ...
	    default:
		  as_bad (_("internal: bad RISC-V opcode (unknown operand type `W%c'): %s %s"),
			  c, opc->name, opc->args);
	    }
	  break;
	}
      case 'X':
	{
	  switch (*p++)
	    {
	      case 'm': used_bits |= ENCODE_T_HEAD_EXT_MIMM(-1U); break;
	      case 'l': used_bits |= ENCODE_T_HEAD_EXT_LIMM(-1U); break;
	      case 'a': used_bits |= ENCODE_T_HEAD_LR_IMM(-1U); break;
	      case '>':
		{
		  int at = -1;
		  if (*p =='@')
		     ...
		     * */
		    break;
		  used_bits |= ENCODE_T_HEAD_IMM(-1U, 6, at);
		  break;
		}
	      case 'I':
		{
	        ...
	        switch (c = *p++)
		  {
		    ...
		    default:
			    as_bad (_("internal: bad RISC-V opcode (unknown operand type `XP%c'): %s %s"),
				    c, opc->name, opc->args);
		  }
		break;
	      case 'M':
  ...
  while (opcodes[i].name)
    {
      ...
      hash_error = hash_insert (hash, name, (void *) &opcodes[i]);
...
md_begin (void)
{
  ...
  hash_reg_names (RCLASS_MR, riscv_mr_names_numeric, NMR);
...
{
  const struct riscv_opcode *mo = NULL;
  ...
  if (riscv_lookup_subset_version (&riscv_subsets, "V", 0, 7) != NULL)
    mo = (struct riscv_opcode *) hash_find (op_v_07_hash, name);
...
vector_macro (struct riscv_cl_insn *ip)
{
  ...
  switch (mask)
      ...
      if (vtemp != 0)
	{
	  ...
	  if (vd == vm
	      || (riscv_lookup_subset_version (&riscv_subsets, "v", 0, 7) != NULL))
	    {
...
my_getVsetvliExpression (expressionS *ep, char *str)
{
  ...
  if (riscv_lookup_subset_version (&riscv_subsets, "V", 0, 7) == NULL
      && (vsew_found || vlmul_found || vta_found || vma_found))
    {
      ep->X_op = O_constant;
  ...
  else if (riscv_lookup_subset_version (&riscv_subsets, "V", 0, 7) != NULL
	   && (vlen_found || vediv_found || vsew_found))
    {
      ep->X_op = O_constant;
      ...
      for (args = insn->args;; ++args)
	{
	  ...
	  switch (*args)
		...
		case '<':
		  ...
		  goto rvc_imm_done;
                case '2':
                  ...
                  if (my_getSmallExpression (imm_expr, imm_reloc, s, p)
                      || imm_expr->X_op != O_constant
                      || !VALID_RVC_UIMM2 (imm_expr->X_add_number))
                    break;
                  ip->insn_opcode |= ENCODE_RVC_UIMM2 (imm_expr->X_add_number);
                  goto rvc_imm_done;
                case '8':
		  if (my_getSmallExpression (imm_expr, imm_reloc, s, p)
		      || imm_expr->X_op != O_constant
		...
		*imm_reloc = BFD_RELOC_RISCV_CALL;
		...
		{
		case 'f':
		  if (arg_lookup (&s, riscv_fli_value,
		      ARRAY_SIZE (riscv_fli_value), &regno))
		    {
		      regno = (regno < 32) ? regno : regno - 32;
		      INSERT_OPERAND (RS1, *ip, regno);
		      continue;
		    }
              ...
              switch (*++args)
		{
		  case '>':
		    {
		      ...
		      INSERT_BITS (ip->insn_opcode, imm_expr->X_add_number,
				   xlen - 1, at);
		      imm_expr->X_op = O_absent;
		      s = expr_end;
		    }
		  ...
		  case 'I':
			...
			imm_expr->X_add_number <<= -shift;
		      ...
		      if (imm_expr->X_op != O_constant
			  || (!sign
			      && !VALID_T_HEAD_EXT_IMM(imm_expr->X_add_number, nbits, at))
			  || (sign
			      && (imm_expr->X_add_number < (0 - pow2(nbits-1))
				  || imm_expr->X_add_number > (pow2(nbits - 1) - 1)))) {
			  break;
		      }
		  ...
		  case 'm':
		    my_getExpression (imm_expr, s);
		    if (imm_expr->X_op != O_constant
		        || !VALID_T_HEAD_EXT_MIMM (imm_expr->X_add_number))
		      break;
		    INSERT_OPERAND (T_HEAD_EXT_MSB, *ip, imm_expr->X_add_number);
		    imm_expr->X_op = O_absent;
		    s = expr_end;
		    continue;
		  case 'l':
		    my_getExpression (imm_expr, s);
		    if (imm_expr->X_op != O_constant
		        || !VALID_T_HEAD_EXT_LIMM (imm_expr->X_add_number))
		      break;
		    INSERT_OPERAND (T_HEAD_EXT_LSB, *ip, imm_expr->X_add_number);
		    imm_expr->X_op = O_absent;
		    s = expr_end;
		    continue;
		  case 'a':
		    my_getExpression (imm_expr, s);
		    if (imm_expr->X_op != O_constant
		        || !VALID_T_HEAD_LR_IMM (imm_expr->X_add_number))
		      break;
		    INSERT_OPERAND (T_HEAD_LR_IMM, *ip, imm_expr->X_add_number);
		    imm_expr->X_op = O_absent;
		    s = expr_end;
		    continue;
		  case 'P': /* RVP */
...
md_assemble (char *str)
{
  ...
  if (riscv_lookup_subset_version (&riscv_subsets, "V", 0, 7) != NULL)
    error = riscv_ip (str, &insn, &imm_expr, &imm_reloc, op_v_07_hash);
...
struct option md_longopts[] =
{
  ...
  {"mwith-thead-id", required_argument, NULL, OPTION_THEAD_ID},
  {"mwithout-thead-id", required_argument, NULL, OPTION_NO_THEAD_ID},
...
md_parse_option (int c, const char *arg)
{
  switch (c)
	...
	riscv_set_abi (64, FLOAT_ABI_QUAD, FALSE);
      else if (strcmp (arg, "lp64v") == 0)
	riscv_set_abi (64, FLOAT_ABI_DOUBLE, FALSE);
      else if (strcmp (arg, "lp64dv") == 0)
	riscv_set_abi (64, FLOAT_ABI_DOUBLE, FALSE);
      else
	return 0;
      break;
...
riscv_after_parse_args (void)
{
  ...
  riscv_set_rvc (FALSE);
  if (riscv_subset_supports ("c") || riscv_subset_supports ("zca"))
    riscv_set_rvc (TRUE);
...
static size_t pcrel_hi_reloc_count = 0;
static size_t pcrel_hi_reloc_max = 0;

static bfd_boolean
riscv_pcrel_hi_reloc_records_alloc(void)
{
  riscv_pcrel_hi_reloc *new_records;
  pcrel_hi_reloc_max += 1024;
  new_records = xmalloc (sizeof(riscv_pcrel_hi_reloc) * pcrel_hi_reloc_max);
  if (!new_records)
    return FALSE;

  if (pcrel_hi_reloc_records_ptr) {
    memcpy(new_records, pcrel_hi_reloc_records_ptr, sizeof(riscv_pcrel_hi_reloc) * pcrel_hi_reloc_count);
    xfree(pcrel_hi_reloc_records_ptr);
  }

  pcrel_hi_reloc_records_ptr = new_records;

  return TRUE;
}

static void
riscv_pcrel_hi_reloc_records_free(void)
{
  if (pcrel_hi_reloc_records_ptr)
    xfree(pcrel_hi_reloc_records_ptr);
  pcrel_hi_reloc_records_ptr = NULL;
}
...
riscv_pcrel_hi_reloc_record (segT seg, bfd_vma addr, symbolS *sym, bfd_vma target)
{
  ...
  if (pcrel_hi_reloc_count == pcrel_hi_reloc_max) {
    if (!riscv_pcrel_hi_reloc_records_alloc())
      return FALSE;
  }
...
md_apply_fix (fixS *fixP, valueT *valP, segT seg)
{
  ...
  switch (fixP->fx_r_type)
    ...
    case BFD_RELOC_32:
	 ...
	 in .eh_frame. Othrewise, the value may be adjusted twice.*/
	  ...
	  && (strcmp (sub_segment->name, ".eh_frame") == 0
	      || (TARGET_MULTIPLE_EH_FRAME_SECTIONS && flag_eh_frame_sections
		  && strncmp (sub_segment->name, ".eh_frame.", 10) == 0))
    ...
    case BFD_RELOC_RISCV_PCREL_HI20:
      if (!riscv_opts.relax && S_IS_LOCAL(fixP->fx_addsy) && fixP->fx_addsy && S_GET_SEGMENT(fixP->fx_addsy) == seg)
      {
        /* Fill in a tentative value to improve objdump readability.  */
        bfd_vma target = S_GET_VALUE (fixP->fx_addsy) + *valP;
    ...
    case BFD_RELOC_RISCV_PCREL_LO12_I:
      {
        ...
        {
          /* Fill in a tentative value to improve objdump readability.  */
          bfd_vma target = res->target;
          bfd_vma value = target - res->address;
...
riscv_md_end (void)
{
  riscv_pcrel_hi_reloc_records_free();
